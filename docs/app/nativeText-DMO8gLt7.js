import { Et as e, Ut as t, j as n, r } from "./paramUtils-BYI4rwyL.js";
...
function a(e, t) {
	...
	return r?.length ? r : n.facetBatches.get(t.facetId) ?? i;
}
function o(e, t, n, r, i) {
	let o = a(e, t);
	if (t.sampleFacetRenderingOptions) r(s(n, t.sampleFacetRenderingOptions), o);
	else if (e.encoders.facetIndex) for (let [t, a] of l(e.encoders.facetIndex, o)) {
		let o = c(e, t);
		o ? r(s(n, {
			locSize: o,
			pixelToUnit: 1
...
function B(e) {
	...
	return {
		configuredRHeadSlope: U(R(e, t.headAngle)),
		configuredRHeadNotchSlope: U(R(e, t.headNotchAngle)),
...
function V(e, t, n, r) {
	let { coords: i, data: a, visibleBounds: o, viewOpacity: s, countOnly: c } = n, l = e.encoders;
	if (a.length == 0) return 0;
	let u = M(i, l, "x", a[0]), d = M(i, l, "y", a[0]), f = [0, 0], p = [0, 0], h = 0;
	for (let e of a) {
		u(e, f), d(e, p);
		let [n, i] = f, [a, _] = p, v = {
			x: n,
			...
			x: i,
			y: _
		}, b = L(l.direction, e), x = b == "reverse" ? y : v, S = b == "reverse" ? v : y, C = ae(S, x), w = Math.hypot(C.x, C.y);
		if (w == 0) continue;
		let T = G(C, 1 / w), E = {
			x: -T.y,
			y: T.x
		}, D = Math.max(P(l.size, e), t.minSize), O = D / 2, k = Math.max(t.headWidth * D, 0) / 2, A = re({
			segmentLength: w,
			headHalfWidth: k,
			stemHalfWidth: t.stem ? O : -O,
		...
		}), j = t.headShape == "open" ? A : Math.min(t.configuredRHeadNotchSlope, A), M = t.headShape == "open" ? D : 0, N = t.headPlacement == "outside" ? ie(k, A, j, M) : 0, F = t.headPlacement == "outside" ? W(S, G(T, N)) : S, I = k * A, R = Math.hypot(k, I), B = t.headShape == "open" && R > 0 ? D * k / R : 0, V = W(F, G(T, -I - B)), H = P(l.strokeWidth, e), U = Math.max(t.stem ? O : 0, k) + H * 2;
		if (!m(o, Math.min(x.x, F.x, V.x), Math.min(x.y, F.y, V.y), Math.max(x.x, F.x, V.x), Math.max(x.y, F.y, V.y), U) || (h++, c)) continue;
		let K = t.headShape == "triangle" || t.headShape == "open" ? t.headShape : "triangle", q = t.stem ? te(x, F, T, E, O, A, t.startNotch) : null, J = q ? [q] : [], oe = q && K == "triangle" && !t.repeatHeads && k <= O, Y = I + M / Math.hypot(A, 1) + H, X = t.repeatHeads ? Math.max((t.headSpacing ?? 0) * D, Y) : Infinity, Z = Math.hypot(F.x - x.x, F.y - x.y);
		for (let e = 0; !oe && !(e > 0 && e + Y - H / 2 > Z); e += X) {
			let n = W(F, G(T, -e));
			if (J.push(K == "open" ? ne(n, T, E, k, A, j, D) : ee(n, T, E, k, A, j)), !t.repeatHeads || X <= 0) break;
		}
		let Q = z(l.stroke(e)) != "none" && P(l.strokeOpacity, e) * s > 0 && H > 0;
		r({
			datum: e,
			boundaryLoops: J.length == 1 || !Q ? J : g(J),
			strokeWidth: H,
			headShapeFallback: K != t.headShape
		});
	}
	return h;
}
...
function re({ segmentLength: e, headHalfWidth: t, stemHalfWidth: n, configuredRHeadSlope: r, configuredRHeadNotchSlope: i, headRepeat: a, headPlacement: o, startNotch: s, minStemLength: c, headShape: l }) {
	...
	return H(n, t, r, i) <= u ? r : u < n * i ? n > 0 ? K(u / n, 0, r) : 0 : K((u + (t - n) * i) / t, 0, r);
}
function H(e, t, n, r) {
...
function q(e, t, n, r) {
	if (r.shape == "arc") {
		...
		let s = se(Z(a, i));
		J(i, a, s, r.maxChordLength, n), r.clampApex && oe(i, a, r.orient, n);
		let c = Q(o, 1 / .75);
		return [
			i,
			X(i, c),
...
function J(e, t, n, r, i) {
	if (n <= r) return n;
	let a = Q(Z(t, e), 1 / n);
	return Y(e, i, 2) ? (ce(t, X(e, Q(a, r))), r) : Y(t, i, 2) ? (ce(e, Z(t, Q(a, r))), r) : n;
}
function oe(e, t, n, r) {
...
function ue(e, t, n, r) {
	...
	for (let e of a) {
		c(e, u), l(e, d);
		let [n, i] = u, [a, p] = d, v = q([n - f, g - a], [i - f, g - p], h, t);
...
function fe(e, t, n, r) {
	let { coords: i, data: a, visibleBounds: o, anchorCullBounds: s } = n, c = e.encoders;
	if (a.length == 0) return 0;
	let l = M(i, c, "x", a[0]), u = M(i, c, "y", a[0]), d = [0, 0], f = [0, 0], h = e.getSemanticThreshold(), g = {
		datum: {},
		shape: "circle",
		x: 0,
		y: 0,
	...
	for (let e of a) {
		if (P(c.semanticScore, e) < h) continue;
		l(e, d), u(e, f);
		let n = d[0] + P(c.dx, e), i = f[0] - P(c.dy, e);
		if (p(s, n, i)) continue;
		let a = Math.sqrt(P(c.size, e)) / 2;
		if (t.inwardStroke && a <= 0) continue;
		let v = L(c.shape, e), y = P(c.strokeWidth, e), b = v == "x" || v == "+", x = t.inwardStroke && !b ? Math.min(y, a) : y, S = t.inwardStroke && !b ? a - x / 2 : a, C = t.inwardStroke ? 0 : y / 2, w = a * Math.SQRT2 + C;
		m(o, n - w, i - w, n + w, i + w) && (_++, g.datum = e, g.shape = v, g.x = n, g.y = i, g.geometryRadius = S, g.angle = P(c.angle, e), g.strokeWidth = x, g.lineShape = b, r(g));
	}
	return _;
}
//#endregion
...
function pe(e, t, n, r, i) {
	...
	switch (e) {
		...
		case "tick-up": return $(i, t - o, n - r, t + o, n), !0;
		case "tick-right": return $(i, t, n - o, t + r, n + o), !0;
		case "tick-down": return $(i, t - o, n, t + o, n + r), !0;
		case "tick-left": return $(i, t - r, n - o, t, n + o), !0;
...
function he(e) {
	...
	return {
		...
		shadow: i,
		hatch: a,
		canPadSeams: a == "none" && i.opacity == 0 && ye(r)
	};
}
function ge(e, t, n, r) {
	let { coords: i, data: a, visibleBounds: o, viewOpacity: s } = n, c = e.encoders;
	if (a.length == 0) return 0;
	let l = c.strokeWidth.constant ? P(c.strokeWidth, a[0]) : 0, u = c.fillOpacity.constant ? P(c.fillOpacity, a[0]) : 0, d = M(i, c, "x", a[0]), f = M(i, c, "y", a[0]), p = [0, 0], h = [0, 0], g = {
		datum: {},
		x: 0,
		y: 0,
		width: 0,
	...
	for (let e of a) {
		d(e, p), f(e, h);
		let n = Math.min(p[0], p[1]), i = Math.min(h[0], h[1]), a = Math.abs(p[1] - p[0]), v = Math.abs(h[1] - h[0]), y = _e(a, t.minWidth), b = _e(v, t.minHeight), x = Math.max(t.minOpacity, y * b);
		a < t.minWidth && (n -= (t.minWidth - a) / 2, a = t.minWidth), v < t.minHeight && (i -= (t.minHeight - v) / 2, v = t.minHeight);
		let S = c.strokeWidth.constant ? l : P(c.strokeWidth, e), C = z(c.fill(e)), w = (c.fillOpacity.constant ? u : P(c.fillOpacity, e)) * s, T = S == 0 && w == 1 && x == 1 && t.canPadSeams && C != "none" ? me : 0;
		T && (n -= T, i -= T, a += T * 2, v += T * 2);
		let E = t.shadow.opacity > 0 ? t.shadow.blur + Math.max(Math.abs(t.shadow.offsetX), Math.abs(t.shadow.offsetY)) : 0;
		m(o, n, i, n + a, i + v, S / 2 + E) && (_++, g.datum = e, g.x = n, g.y = i, g.width = a, g.height = v, g.opacityFactor = x, g.strokeWidth = S, g.fill = C, g.fillOpacity = w, ve(t.cornerRadii, a, v, g.radii), r(g));
	}
	return _;
}
function _e(e, t) {
...
function Ce(r, i, a, o) {
	...
	let h = m.size.constant ? P(m.size, c[0]) : 0, g = m.angle.constant ? P(m.angle, c[0]) : 0, _ = M(s, m, "x", c[0]), v = M(s, m, "y", c[0]), y = r.encoding.text, b = "format" in y ? t(y.format) : (e) => e, x = [0, 0], S = [0, 0], C = {
		width: 0,
		height: 0
	}, w = {
		x: 0,
	...
	for (let t of c) {
		let a = b(m.text(t)), c = e(a) ? a : a === null ? "" : "" + a;
		if (!c) continue;
		_(t, x), v(t, S);
		let [y, j] = x, [M, N] = S, F = m.size.constant ? h : P(m.size, t), I = m.angle.constant ? g : P(m.angle, t);
		if (i.logoLetters) {
			let e = F, n = F;
			if (O && (e = j - y, y = (y + j) / 2), k && (n = M - N, M = (M + N) / 2), !e || !n || p(u, y, M) || !we(l, y, M, Math.abs(e), Math.abs(n), "center", "middle", I, i.dx, i.dy)) continue;
			let a = r.font.metrics.getChar(c[0]), s = n * r.font.metrics.common.base * (a.height + 10) / (a.height * a.height);
			A++, D.datum = t, D.text = c, D.x = y, D.y = M, D.size = 1, D.width = 1, D.angle = I, D.fadeOpacity = 1, D.scale = 1, E.width = e, E.heightScale = s, D.logoScale = E, D.multiCharacterLogo = c.length > 1, o(D);
			continue;
		}
		let L = r.font.metrics.measureWidth(c, F);
		Te(L, F, I, C), O || k ? De(d, f, I, w) : (w.x = Oe[d], w.y = ke[f]);
		let R = 1;
		if (O && (Ee(Math.min(y, j), Math.max(y, j), C.width, i.paddingX, w.x, i.flushX, s.x, s.x2, T), y = T.position, R *= T.scale), k && (Ee(Math.min(M, N), Math.max(M, N), C.height * R, i.paddingY, w.y, i.flushY, s.y, s.y2, T), M = T.position, R *= T.scale), p(u, y, M)) continue;
		let z = 1;
		if (R < 1) {
			if (!i.squeeze || R < 3 / F) continue;
			z = n(3 / F, 6 / F, R);
		}
		let B = F * R, V = L * R;
		we(l, y, M, V, B, d, f, I, i.dx, i.dy) && (A++, D.datum = t, D.text = c, D.x = y, D.y = M, D.size = B, D.width = V, D.angle = I, D.fadeOpacity = z, D.scale = R, D.logoScale = void 0, D.multiCharacterLogo = !1, o(D));
	}
	return A;
}
function we(e, t, n, r, i, a, o, s, c, l) {
	let u = t + c - (Oe[a] + 1) / 2 * r, d = n + l - (ke[o] + 1) / 2 * i, f = u + r, p = d + i;
	if (!s) return m(e, u, d, f, p, 1);
	let h = s * Math.PI / 180, g = Math.sin(h), _ = Math.cos(h), v = t + (u - t) * _ - (d - n) * g, y = n + (u - t) * g + (d - n) * _, b = t + (f - t) * _ - (d - n) * g, x = n + (f - t) * g + (d - n) * _, S = t + (f - t) * _ - (p - n) * g, C = n + (f - t) * g + (p - n) * _, w = t + (u - t) * _ - (p - n) * g, T = n + (u - t) * g + (p - n) * _;
	return m(e, Math.min(v, b, S, w), Math.min(y, x, C, T), Math.max(v, b, S, w), Math.max(y, x, C, T), 1);
}
function Te(e, t, n, r) {
...
function De(e, t, n, r) {
	let i = Oe[e], a = -ke[t], o = ((n + 45) % 360 + 360) % 360, s, c;
...
function Pe(e) {
	let t = e ?? "Lato";
	return [t, ...Me.filter((e) => e.toLowerCase() != t.toLowerCase())].map(Ie).join(", ");
}
function Fe(e, t) {
	switch (e) {
...
function Ie(e) {
	let t = e.toLowerCase();
	return Ne.has(t) ? t : `'${e.replaceAll("\\", "\\\\").replaceAll("'", "\\'")}'`;
}
//#endregion
export { u as C, o as E, d as S, m as T, P as _, Ce as a, R as b, he as c, de as d, fe as f, V as g, B as h, Se as i, ge as l, ue as m, Fe as n, be as o, le as p, je as r, xe as s, Pe as t, pe as u, A as v, f as w, z as x, j as y };
