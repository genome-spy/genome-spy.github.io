import { $ as e, Yt as t, b as n, fn as r } from "./clipOptions-BWHr3aTa.js";
...
function R(e, t) {
	return n(t) ? e.unitView.paramRuntime.evaluateAndGet(t.expr) : t;
}
function z(e) {
...
function le(e, t, n = !1) {
	let r = R(e, e.properties.arcFadingDistance);
	return t != "arc" && t != "dome" || r === !1 || r[0] <= 0 || r[1] <= 0 || n && R(e, e.properties.noFadingOnSecondPass) ? !1 : r;
}
function ue(e, t) {
	let n = [], r = (r) => {
		for (let i = 0; i <= se; i++) {
			let a = i / se, o = r ? t + (e - t) * a : e + (t - e) * a, s = r ? (t - o) / (2 * t) : (t + o) / (2 * t), c = (o - e) / (t - e);
			n.push({
				offset: s,
				opacity: 1 - de(c)
			});
...
function fe(e, t, n, r) {
	if (r.shape == "arc") {
		...
		let s = ge(X(a, i));
		pe(i, a, s, r.maxChordLength, n), r.clampApex && me(i, a, r.orient, n);
		let c = Z(o, 1 / .75);
		return [
			i,
			Y(i, c),
...
function pe(e, t, n, r, i) {
	if (n <= r) return n;
	let a = Z(X(t, e), 1 / n);
	return he(e, i, 2) ? (_e(t, Y(e, Z(a, r))), r) : he(t, i, 2) ? (_e(e, X(t, Z(a, r))), r) : n;
}
function me(e, t, n, r) {
...
function ye(e, t, n, r) {
	...
	for (let e of a) {
		c(e, u), l(e, d);
		let [n, i] = u, [a, p] = d, v = fe([n - f, g - a], [i - f, g - p], h, t);
...
function Te(e, t, n, r) {
	...
	for (let e = c; e < l; e++) {
		let n = a[e];
		y(n, x), b(n, S);
		let i = Math.min(x[0], x[1]), c = Math.min(S[0], S[1]), l = Math.abs(x[1] - x[0]), T = Math.abs(S[1] - S[0]), E = Ee(l, t.minWidth), D = Ee(T, t.minHeight), O = Math.max(t.minOpacity, E * D);
		l < t.minWidth && (i -= (t.minWidth - l) / 2, l = t.minWidth), T < t.minHeight && (c -= (t.minHeight - T) / 2, T = t.minHeight);
		let k = d ? p : P(u.strokeWidth, n), A = g ? _ : z(u.fill(n)), j = (f ? h : P(u.fillOpacity, n)) * s, M = k == 0 && j == 1 && O == 1 && t.canPadSeams && A != "none" ? Ce : 0;
		M && (i -= M, c -= M, l += M * 2, T += M * 2), m(o, i, c, i + l, c + T, k / 2 + v) && (w++, C.datum = n, C.x = i, C.y = c, C.width = l, C.height = T, C.opacityFactor = O, C.strokeWidth = k, C.fill = A, C.fillOpacity = j, t.hasCornerRadii && De(t.cornerRadii, l, T, C.radii), r(C));
	}
	return w;
}
function Ee(e, t) {
...
function Me(n, i, a, o) {
	let { coords: s, data: c, visibleBounds: l, anchorCullBounds: u } = a, { align: d, baseline: f } = i, m = n.encoders;
	if (c.length == 0) return 0;
	let h = m.size.constant ? P(m.size, c[0]) : 0, g = m.angle.constant ? P(m.angle, c[0]) : 0, _ = M(s, m, "x", c[0]), v = M(s, m, "y", c[0]), y = n.encoding.text, b = "format" in y ? r(y.format) : (e) => e, x = [0, 0], S = [0, 0], C = {
		width: 0,
		height: 0
	}, w = {
		x: 0,
	...
	for (let r of c) {
		let a = b(m.text(r)), c = t(a) ? a : a === null ? "" : "" + a;
		if (!c) continue;
		_(r, x), v(r, S);
		let [y, M] = x, [N, F] = S, I = m.size.constant ? h : P(m.size, r), L = m.angle.constant ? g : P(m.angle, r);
		if (i.logoLetters) {
			let e = I, t = I;
			if (k && (e = M - y, y = (y + M) / 2), A && (t = N - F, N = (N + F) / 2), Ne(D, y, N, Math.abs(e), Math.abs(t), "center", "middle", L, i.dx, i.dy), !e || !t || p(u, y, N) || !Pe(l, D)) continue;
			let a = n.font.metrics.getChar(c[0]), s = t * n.font.metrics.common.base * (a.height + 10) / (a.height * a.height);
			j++, O.datum = r, O.text = c, O.x = y, O.y = N, O.size = 1, O.width = 1, O.angle = L, O.fadeOpacity = 1, O.scale = 1, E.width = e, E.heightScale = s, O.logoScale = E, O.multiCharacterLogo = c.length > 1, o(O);
			continue;
		}
		let R = n.font.metrics.measureWidth(c, I);
		Fe(R, I, L, C), k || A ? Le(d, f, L, w) : (w.x = Re[d], w.y = ze[f]);
		let z = 1;
		if (k && (Ie(Math.min(y, M), Math.max(y, M), C.width, i.paddingX, w.x, i.flushX, s.x, s.x2, T), y = T.position, z *= T.scale), A && (Ie(Math.min(N, F), Math.max(N, F), C.height * z, i.paddingY, w.y, i.flushY, s.y, s.y2, T), N = T.position, z *= T.scale), p(u, y, N)) continue;
		let B = 1;
		if (z < 1) {
		...
		let V = I * z, H = R * z;
		Ne(D, y, N, H, V, d, f, L, i.dx, i.dy), Pe(l, D) && (j++, O.datum = r, O.text = c, O.x = y, O.y = N, O.size = V, O.width = H, O.angle = L, O.fadeOpacity = B, O.scale = z, O.logoScale = void 0, O.multiCharacterLogo = !1, o(O));
	}
	return j;
}
function Ne(e, t, n, r, i, a, o, s, c, l) {
	let u = t + c - (Re[a] + 1) / 2 * r, d = n + l - (ze[o] + 1) / 2 * i, f = u + r, p = d + i, m = s * Math.PI / 180, h = Math.sin(m), g = Math.cos(m);
...
function Le(e, t, n, r) {
	let i = Re[e], a = -ze[t], o = ((n + 45) % 360 + 360) % 360, s, c;
...
function We(e) {
	let t = e ?? "Lato";
	return [t, ...He.filter((e) => e.toLowerCase() != t.toLowerCase())].map(Ke).join(", ");
}
function Ge(e, t) {
	switch (e) {
...
function Ke(e) {
	let t = e.toLowerCase();
	return Ue.has(t) ? t : `'${e.replaceAll("\\", "\\\\").replaceAll("'", "\\'")}'`;
}
//#endregion
...
//#endregion
export { f as A, P as C, z as D, R as E, o as M, c as N, d as O, H as S, j as T, ce as _, je as a, U as b, Ae as c, Se as d, be as f, ue as g, ye as h, Ve as i, m as j, u as k, we as l, ve as m, We as n, Me as o, xe as p, Ge as r, ke as s, qe as t, Te as u, le as v, A as w, V as x, B as y };
